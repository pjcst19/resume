...
public class MysqlIO {
    ...
    /**
     * We need to have a 'marker' for all-zero datetimes so that ResultSet can
     * decide what to do based on connection setting
     */
     ...
     *
     * @throws SQLException if the network fails while skipping the packet.
     */
    ...
    /**
     * Fill the {@link MysqlIO#authenticationPlugins} map. First this method
     * fill the map with instances of {@link MysqlOldPasswordPlugin}, {@link MysqlNativePasswordPlugin},
     * {@link MysqlClearPasswordPlugin} and {@link Sha256PasswordPlugin}. Then
     * it gets instances of plugins listed in "authenticationPlugins" connection
     * property by
     * {@link Util#loadExtensions(Connection, Properties, String, String, ExceptionInterceptor)}
     * call and adds them to the map too.
     *
     * The key for the map entry is getted by
     * {@link AuthenticationPlugin#getProtocolPluginName()}. Thus it is possible
     * to replace built-in plugin with custom one, to do it custom plugin should
     * return value "mysql_native_password", "mysql_old_password",
     * "mysql_clear_password" or "sha256_password" from it's own
     * getProtocolPluginName() method.
     *
     * All plugin instances in the map are initialized by
     * {@link Extension#init(Connection, Properties)} call with this.connection
     * and this.connection.getProperties() values.
     *
     * @throws SQLException
     */
    ...
    /**
     * Add plugin to {@link MysqlIO#authenticationPlugins} if it is not disabled
     * by "disabledAuthenticationPlugins" property, check is it a default
     * plugin.
     *
    ...
    /**
     * Get authentication plugin instance from
     * {@link MysqlIO#authenticationPlugins} map by pluginName key. If such
     * plugin is found it's {@link AuthenticationPlugin#isReusable()} method is
     * checked, when it's false this method returns a new instance of plugin and
...
//
    ...
    /**
     * Send a command to the MySQL server If data is to be sent with command, it
     * should be put in extraData.
     *
     * Raw packets can be sent by setting queryPacket to something other than
     * null.
     *
