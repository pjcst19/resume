...
public interface StatementInterceptorV2 extends Extension {
     ...
     * specific to an interceptor <strong>must</strong> be passed via setURL()
     * on the DataSource. StatementInterceptor properties are not exposed via
     * accessor/mutator methods on DataSources.
     *
     * @throws SQLException should be thrown if the the StatementInterceptor can
     * not initialize itself.
     */
    public abstract void init(Connection conn, Properties props) throws SQLException;

    /**
     * Called before the given statement is going to be sent to the server for
     * processing.
     *
     * Interceptors are free to return a result set (which must implement the
     * interface com.mysql.jdbc.ResultSetInternalMethods), and if so, the server
     * will not execute the query, and the given result set will be returned to
     ...
     * @param interceptedStatement the actual statement instance being
     * intercepted
     * @param connection the connection the statement is using (passed in to
     * make thread-safe implementations straightforward)
     *
    ...
    /**
     * Should the driver execute this interceptor only for the "original"
     * top-level query, and not put it in the execution path for queries that
     * may be executed from other interceptors?
     *
    ...
    /**
     * Called after the given statement has been sent to the server for
     * processing, instead of the StatementAware postProcess() of the earlier
     * api.
     *
     ...
     * must implement the interface com.mysql.jdbc.ResultSetInternalMethods).
     *
     * This method will be called while the connection-level mutex is held, so
     * it will only be called from one thread at a time.
     *
