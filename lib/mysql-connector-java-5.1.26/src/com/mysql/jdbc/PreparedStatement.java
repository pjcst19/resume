...
public class PreparedStatement extends com.mysql.jdbc.StatementImpl implements
    ...
    class ParseInfo {
        ...
        /**
         * Represents the "parsed" state of a client-side prepared statement,
         * with the statement broken up into it's static and dynamic (where
         * parameters are bound) parts.
         */
        ...
        /**
         * Returns a ParseInfo for a multi-value INSERT for a batch of size
         * numBatch (without parsing!).
         */
         ...
         * used by server-side prepared statements when re-writing batch
         * INSERTs.
         */
         ...
         * a visitor pattern here, because the if {}s make computing a size for
         * the resultant byte[][] make this too complex, and we don't
         * necessarily want to use a List for this, because the size can be
         * dynamic, and thus we'll not be able to guess a good initial size for
         * an array-based list, and it's not efficient to convert a LinkedList
         * to an array.
         */
    ...
    /**
     * Only used by statement interceptors at the moment to provide
     * introspection of bound values
     */
     ...
     * in the batch. The array is ordered according to the order in which
     * commands were inserted into the batch
     *
    ...
    protected int[] executeBatchedInserts(int batchTimeout) throws SQLException {
        synchronized (checkClosed().getConnectionMutex()) {
            ...
            try {
                try {
                    batchedStatement
                            = /* FIXME -if we ever care about folks proxying our MySQLConnection */ prepareBatchedInsertSQL(locallyScopedConn, numValuesPerBatch);
     ...
     * this is the number of digits after the decimal. For all other types this
     ...
     * the first '?' parameter marker, it's only syntax, but JDBC counts it as
