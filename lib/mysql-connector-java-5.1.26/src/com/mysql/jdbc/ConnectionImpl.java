...
public class ConnectionImpl extends ConnectionPropertiesImpl implements
    ...
    /**
     * Map for Java charsets of user defined charsets. We can't map them
     * statically, because they can be different for different server URLs.
     */
    private static final Map<String, Map<Integer, String>> serverJavaCharsetByUrl = new HashMap<String, Map<Integer, String>>();
    /**
     * Map for user defined charsets. We can't map them statically, because they
     * can be different for different server URLs.
     */
...
//	private long masterFailTimeMillis = 0L;
    ...
    /**
     * The comment (if any) that we'll prepend to all statements sent to the
     * server (to show up in "SHOW PROCESSLIST")
     */
    ...
    /**
     * Clobbers the physical network connection and marks this connection as
     * closed.
     *
     * @throws SQLException
     */
     ...
     * @throws SQLException if an exception happens while sending 'SET NAMES' to
    ...
    public String getCharsetNameForIndex(int charsetIndex)
        ...
        if (charsetIndex != MysqlDefs.NO_CHARSET_INFO) {
            try {
                ...
                if (charsetName == null) {
                    charsetName = CharsetMapping.INDEX_TO_CHARSET[charsetIndex];
                }
    ...
    public int getMaxBytesPerChar(Integer charsetIndex, String javaCharsetName) throws SQLException {
        ...
        try {
            ...
            // checking against static maps if no custom charset found
            if (charset == null) {
                charset = CharsetMapping.STATIC_INDEX_TO_MYSQL_CHARSET_MAP.get(charsetIndex);
            }
            ...
            if (mblen == null) {
                mblen = CharsetMapping.STATIC_CHARSET_TO_NUM_BYTES_MAP.get(charset);
            }
            if (mblen == null) {
                mblen = CharsetMapping.STATIC_4_0_CHARSET_TO_NUM_BYTES_MAP.get(charset);
            }
    ...
    public String getServerCharacterEncoding() {
        if (this.io.versionMeetsMinimum(4, 1, 0)) {
            ...
            if (charset == null) {
                charset = CharsetMapping.STATIC_INDEX_TO_MYSQL_CHARSET_MAP.get(this.io.serverCharsetIndex);
            }
    ...
    /**
     * Has the default autocommit value of 0 been changed on the server via
     * init_connect?
     *
    ...
    /**
     * Is this connection connected to the first host in the list if there is a
     * list of servers in the URL?
     *
    ...
    /**
     * Tests to see if the connection is in Read Only Mode. Note that prior to
     * 5.6, we cannot really put the database in read only mode, but we pretend
     ...
     * we can by returning the value of the readOnly flag
     *
     * @param useSessionStatus in some cases, for example when restoring
     * connection with autoReconnect=true, we can rely only on saved readOnly
     * state, so use useSessionStatus=false in that case
     *
     * @return true if the connection is read only
     * @exception SQLException if a database access error occurs
     */
     ...
     *
     * This method is synchronized by the caller on getMutex(), so if calling
     * this method from internal code in the driver, make sure it's synchronized
     * on the mutex that guards communication with the server.
     *
     * @param sql the query that is the key to the cache
     *
     * @return metadata cached for the given SQL, or none if it doesn't exist.
     */
     ...
     * server.
     *
     * @return the comment that will be prepended to all statements sent to the
     * server.
     */
     ...
     *
     * @param comment the comment that will be prepended to all statements sent
     * to the server.
     */
     ...
     * as closed and the <code>Executor</code> that was passed as a parameter to
     ...
     *
     * @param executor The <code>Executor</code> implementation which will be
     * used by <code>abort</code>.
