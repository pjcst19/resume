...
/**
 * This interface contains methods that are considered the "vendor extension" to
 * the JDBC API for MySQL's implementation of java.sql.Connection.
 *
 * For those looking further into the driver implementation, it is not an API
 * that is used for plugability of implementations inside our driver (which is
 * why there are still references to ConnectionImpl throughout the code).
 *
 * @version $Id: $
 *
 */
public interface Connection extends java.sql.Connection, ConnectionProperties {
     ...
     * (irregardless of the configuration property 'useServerPrepStmts') with
     * the same semantics as the java.sql.Connection.prepareStatement() method
    ...
    /**
     * Returns the number of statements active on this connection, which haven't
     * been .close()d.
     */
    public abstract int getActiveStatementCount();

    /**
     * Reports how long this connection has been idle. This time (reported in
    ...
    /**
     * Returns the TimeZone that represents the configured timezone for the
     * server.
     */
    ...
    /**
     * Has this connection tried to execute a query on the "master" server
     * (first host in a multiple host list).
     */
    ...
    /**
     * Does this connection have the same resource name as the given connection
     * (for XA)?
     *
     * @param c
     * @return
    ...
    /**
     * Detect if the connection is still good by sending a ping command to the
     * server.
     *
    ...
    /**
     * Prepares a statement on the server (irregardless of the configuration
     * property 'useServerPrepStmts') with the same semantics as the
     * java.sql.Connection.prepareStatement() method with the same argument
     * types.
     *
     * @see java.sql.Connection#prepareStatement(String)
     */
    ...
    /**
     * Does the server this connection is connected to support quoted isolation
     * levels?
     */
    public abstract boolean supportsIsolationLevel();

    /**
     * Does the server this connection is connected to support quoted
     * identifiers?
     */
    ...
    /**
     * Does the server this connection is connected to meet or exceed the given
     * version?
     */
    ...
    /**
     * Returns the -session- value of 'auto_increment_increment' from the server
    ...
    /**
     * Is the server this connection is connected to "local" (i.e. same host) as
     * the application?
     *
     * @return
     */
